	...
	<?php 
		while($data = mysqli_fetch_array($query))
		{
			 // $rumus = $data['protokol'] . " -" . $data['versi'] . " -c " . $data['rocomunity'] . " " . $data['ip'] . " " . $data['mib_name'];
		    	...
		    	$ip = $data['ip'];
		    	$mib = $data['mib_name'];
				// exec("/var/www/html/modpoll/linux_arm-eabihf/modpoll -m enc -t3 -a 7 -r 2 -c 1 -1 192.168.0.203",$output);
		    	$rumus1 =exec("/var/www/html/modpoll/linux_arm-eabihf/$protokol -m $versi -t3 -a $recomunity -r $mib -c 1 -1 $ip",$output);

        		$modbus = (substr($output[10],5,3))/10;
				
		    
			//echo $rumus1;
			$posisi = strpos($rumus1,": ");
			$posisi = intval($posisi+2);
			$snmp = substr($rumus1,$posisi,strlen($rumus1));
	?>
            	<tr>
		    <td p align="center" bgcolor="#FFFFFF"><?php echo $data['id']; ?></td>
                    ...
                    <td p align="center" bgcolor="#FFFFFF"><?php echo $data['output']; ?></td>
